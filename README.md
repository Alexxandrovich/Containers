...
Реализация собственной библиотеки, содержащей основные стандартные контейнерные классы языка С++: `list` (список), `queue` (очередь), `set` (множество), `stack` (стек) и `vector` (вектор). Все контейнеры являются шаблонными классами. Реализации представляют собой весь набор стандартных методов и атрибутов для работы с элементами, проверкой заполненности контейнера и итерирования.
